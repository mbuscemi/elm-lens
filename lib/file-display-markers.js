...
export default class FileDisplayMarkers {
    ...
    generate(rootElement, expression, projectIsProcessed, fileIsReprocessing) {
        const range = this.generateMarkerRange(expression.name, expression.lineNumber);
        const decorationOptions = this.generateDecorationOptions(rootElement, expression.elementId);
   ...
   generateDecorationOptions(rootElement, elementId) {
       ...
       // console.log($(rootElement).find("#" + elementId).get(0));
       const element = $(rootElement).find("#" + elementId).get(0) || document.createElement('div');
       const safeElement = element.cloneNode(true);
       return { type: "block", position: "head", item: safeElement }
   }
