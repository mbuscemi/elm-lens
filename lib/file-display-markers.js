...
export default class FileDisplayMarkers {
    ...
    generate(expression, projectIsProcessed) {
        const range = this.generateMarkerRange(expression.name, expression.lineNumber);
        const decorationOptions = this.generateDecorationOptions(projectIsProcessed, expression.isExposed, expression.numInternalRefs, expression.numExternalRefs);
   ...
   generateDecorationOptions(projectIsProcessed, isExposed, numInternalRefs, numExternalRefs) {
