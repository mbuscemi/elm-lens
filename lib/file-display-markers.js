...
export default class FileDisplayMarkers {
    ...
    constructor(textEditor, referencePanelView) {
    ...
    generateDecorationOptions(
        ...
        if (specialType === "ElmProgram") {
            ...
            if (numInternalRefs > 0) {
                $(internalRefSpan).on("click", () => this.referencePanelView.toggle());
            }
            ...
            if (numExternalRefs > 0) {
                $(externalRefSpan).on("click", () => this.referencePanelView.toggle());
            }
