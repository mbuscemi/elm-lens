        ...
        if (specialType === "ElmProgram") {
            ...
            if (numInternalRefs > 0) {
                $(internalRefSpan).on("click", () => {
                    this.elm.ports.setReferencePanel.send([this.textEditor.getPath(), name, false]);
                    this.referencePanelView.toggle();
                });
            }
            ...
            if (numExternalRefs > 0) {
                $(externalRefSpan).on("click", () => {
                    this.elm.ports.setReferencePanel.send([this.textEditor.getPath(), name, true]);
                    this.referencePanelView.toggle();
                });
            }
