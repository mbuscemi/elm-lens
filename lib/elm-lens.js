...
export default {
    ...
    activate(state) {
        ...
        if (activeEditorFacade.isElmFile()) {
            (new ElmFile(this.elm, activeEditorFacade.getFilePath())).process(() => {
                this.elm.ports.fileMarkupRequest.send(activeEditorFacade.getFilePath());
    ...
    processMarkupRequest(editorPath, fileDisplayMarkers) {
        return (data) => {
            if (data.fileName === editorPath) {
                console.log(data);
                fileDisplayMarkers.reset();
                data.expressions.forEach(expression => fileDisplayMarkers.generate(expression, this.batch));
            }
        };
    },
