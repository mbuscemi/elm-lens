...
export default {
    ...
    activate(state) {
        ...
        this.elm.ports.registerProjectFiles.send(this.elmFiles);
        ...
        let elmFileObjects = this.elmFiles.map(filePath => new ElmFile(this.elm, filePath));
        if (activeEditor && activeEditorFacade.isElmFile()) {
            this.processFile(activeEditorFacade, () => this.batch.process(elmFileObjects, 2, 100));
        } else {
            this.batch.process(elmFileObjects, 2, 100);
        }
