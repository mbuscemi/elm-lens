...
export default {
    ...
    activate(state) {
        ...
        this.elm = elmApp.Main.embed(this.rootElement);
        ...
        if (activeEditor && activeEditorFacade.isElmFile()) {
            this.processFile(activeEditorFacade, () => { this.batch.process(this.elmFiles, 2, 100); });
        } else {
    ...
    processMarkupRequest(editorPath, fileDisplayMarkers) {
        return (data) => {
            if (data.fileName === editorPath) {
                console.log(data);
                fileDisplayMarkers.reset();
                data.expressions.forEach(expression => fileDisplayMarkers.generate(this.rootElement, expression, data.projectIsProcessed, data.fileIsReprocessing));
            }
        };
    },
